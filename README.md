...
<head>
  ...
  <meta name="description" content="Teil Danielle — auteure de 'Quand les stars passent à table' (1999) et 'Saint-Étienne et le théâtre' (1990). Page officielle — résumés, couvertures et contact." />
  <meta name="keywords" content="Teil Danielle, écrivaine, Saint-Étienne, théâtre, Quand les stars passent à table, Marco Cruciani" />
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* Responsive */
    @media (max-width:720px){.cards{grid-template-columns:1fr}.cover{height:220px}.title{font-size:26px}}
  </style>
</head>
<body>
  <div class="wrap" role="main">
    <header>
      ...
      <div>
        <div class="lang-toggle" aria-hidden="false">
          <button class="lang" id="btn-fr">FR</button>
          <button class="lang" id="btn-en">EN</button>
        </div>
      </div>
    </header>

    <section class="cards" aria-label="Ouvrages">
      <article class="card" id="book1">
        <div class="cover"><img src="Stars.png" alt="Couverture — Quand les stars passent à table" /></div>
        <h3 class="title-book">Quand les stars passent à table</h3>
        <div class="meta"><span class="pub-date">Septembre 1999</span> — <span class="publisher">Indépendant</span></div>
        <div class="desc" id="desc1">Biographie d'un célèbre restaurateur, Marco Cruciani, qui recevait à sa table les stars du spectacle. Chacune d'entre elles lui laissaient une signature sur son livre d'or, qui comporte aujourd'hui plus de 3000 signatures.</div>
      </article>

      <article class="card" id="book2">
        <div class="cover"><img src="ST-etienne-theatre.png" alt="Couverture — Saint-Étienne et le théâtre" /></div>
        <h3 class="title-book">Saint-Étienne et le théâtre</h3>
        <div class="meta"><span class="pub-date">Octobre 1990</span> — <span class="publisher">Éditions Xavier Lejeune</span></div>
    ...
    <footer>
      ...
      <div style="margin-top:8px;color:var(--muted);font-size:13px">Fichier HTML statique — déposer <strong>page_auteur_teil.html</strong> et les images <em>Stars.png</em> et <em>ST-etienne-theatre.png</em> à la racine du repo GitHub. Active GitHub Pages pour publier.</div>
    </footer>
  </div>

  <script>
    // Bilingual content (FR/EN). Keep English phrases minimal and accurate.
    const translations = {
      en: {
    ...
    function setLang(lang){
      ...
      document.querySelector('#book1 .title-book').textContent = translations[lang]['title-book-1'];
      document.getElementById('desc1').textContent = translations[lang].desc1;
      document.querySelector('#book2 .title-book').textContent = translations[lang]['title-book-2'];
